...
** CANCELLED Literate Baking
   CLOSED: [2024-10-19 Sat 16:19]
:PROPERTIES:
...
** CANCELLED Streaming Folk Music
   CLOSED: [2024-10-19 Sat 16:19]
:PROPERTIES:
...
#+hugo: more
** CANCELLED What's the saw about the best being the enemy of the good?
   CLOSED: [2024-10-19 Sat 16:19]
:PROPERTIES:
:export_file_name: 20230602-what-s-the-saw-about-the-best-being-the-enemy-of-the-good
:END:

[[https://apps.apple.com/gb/app/loopy-pro-looper-daw-sampler/id1492670451][Loopy Pro]] is bloody awesome. It's a looper app for iDevices (and eventually for the mac) by the developer of Loopy HD (which I'm not linking because, seriously, you want Loopy Pro). What makes it so awesome is its flexibility. If you look for it on YouTube, you'll find people using it in a bewildering number of ways.

I use it primarily for singing chorus harmonies when I'm streaming folk songs on Friday night. I set it up so that, after the first verse, I record the tune, next time through I add a high harmony, then a low harmony, then maybe a doubling of the tune, then I search for another harmony or counter melody, and so on until I reach the end of the song. I used to use Logic Pro on my Mac to do the same thing, it's just easier in Loopy.
...
** CANCELLED Thinking about gear
   CLOSED: [2024-10-19 Sat 16:19]

A friend put up an 'all the gear I use in 2024' and of course, now I'm doing the same thing. I'm a sheep, I tell you, a sheep!

Also, I'm hoping that by writing this all down and and thinking about what works and what doesn't, I'll have a better sense of how to prioritise saving for a few upgrades...
...
At the centre of my streaming set up is a Blackmagic ATEM Mini Pro ISO, which is a lot of name for a surprisingly small box. At the most basic level, I use it to switch between four HDMI inputs and stream that to the internet through its ethernet connection. But also: I'm recording all four inputs to an attached SSD, bringing in a view of my looper in split screen and overlaying motion graphics (the little [[https://ko-fi.com/pdcawley][Ko-Fi]] gadget on my life streams, for instance). It also outputs HDMI to a display that which lets me, independently of what's been streamed, preview a specific camera or see a multiview of all of them. It's great.

It's also kind of limiting. Four inputs should surely be enough for someone who has a single camera and maybe one guest over zoom, no? Well… you'd think wouldn't you? But what I'd really like is two HDMI outputs to let me take advantage of my teleprompter display as well more conveniently. The digital video effects (DVE) engine I use to do the split screen is a bit tricksy because it can only manipulate one stream of video - if I want to put my guest on the right hand side of the display, I need to make sure my camera shot is set up so I'm sat on the left hand side of the shot because I can't do any cropping or scaling of that camera; the DVE engine is fully occupied already. And four inputs just isn't enough; Right now my inputs are:

1. Primary camera
2. Mac Primary display
3. Mac Secondary display
4. iPad Pro Mirror
5. Raspberry Pi Primary display (Overlays)
6. Raspberry Pi Secondary display (Media playback)
7. Gaming PC Primary display
8. Gaming PC Secondary display


The rack case my streaming rig lives in has a physical HDMI patch bay set up so I can choose which four sources I can route to the switcher on a case by case basis, but life would be a /lot/ easier if I could just plug all that in once and have done with it.

Which is why the gadget that sits in prime position on my wishlist is the ATEM Mini Extreme ISO. You can think of it as two ATEM Minis in a single case, with a couple of extra bells and whistles, all of which would make my life easier (like a 3.5mm audio out jack so I could monitor the sound going to my stream!). It has 8 inputs, two HDMI outputs, two USB sockets (so I could plug the SSD into one and connect the other to my Mac and treat the ATEM as a webcam, very handy for Zoom) and a built in 'SuperSource' which would make setting up various multicam split screen shots and transitions a breeze. The only catch is, it's £1,120 new, so I'll be saving a while. I don't /need/ it, I can already do everything I need to do the gear I have, so it sits on the back burner.

****** Source 1 - A Nikon D810 rig
Believe me, if you're interested in streaming, don't do what I do. I'm using an excellent stills camera in it's not all that great video camera because that's the camera I had and running that through a cheap capture card was /so much/ better quality than my (decent for the time) webcam that it was like night and day. The DSLR or mirrorless camera you have already is substantially cheaper than anything else comparable you might buy, so if it'll give you a clean HDMI feed or work as a webcam, use that first. The camera is strapped into a Glide Gear Teleprompter (but if I were starting today, I'd probably buy the Elgato Telepompter) which sits on a cheap Manfrotto video head on a cheap levelling head on a Manfrotto 055 aluminium tripod. I have a 10' field monitor providing the teleprompter display, which gets its signal from a dumb HDMI selector switch so I can switch between seeing the camera output or the mac's secondary display. I could use at least one more video splitter so I could add another choice to those (probably the iPad mirror).

As a stills camera, the D810 is magnificent. As a video camera… it's got a really nice sensor and the lens is great. Video autofocus on the 810 is terrible, but I work around that by not moving back and forward very much. I would love to replace it with something like a BlackMagic Micro Studio camera which has some nice capabilities when paired with ATEM gear and which would let me shoot stills (and B Roll) with the 810 much more conveniently. Again, it's spendy though, so onto the lottery wishlist it goes.

****** Sources 2 & 3 - Mac Mini M1
What can I say? The Apple silicon Mac Minis are astonishingly capable, great value, and they fit in half a 1U shelf. If Apple made one with a 12--15V barrel jack power input so I could potentially run whole streaming rig off a few big old V-Mount batteries, I might consider upgrading, but otherwise it's perfect. No notes.

****** Source 4 -- 10″ M1 iPad Pro
This runs Loopy Pro and controls the ATEM. I've recently treated it to a Reloop powered USB hub/stand (can't say enough nice things about that particular gadget). For live performance where I don't care about capturing the iPad's display, I connect direct to the hub, but when I'm streaming, I go via the Apple USB-C Digital AV Multi-port Adaptor (yes, I /know/ they're expensive, but I've thrown away so many cheap options that purported to do what it does). It's powered by a 90W USB-C PD PSU, which is overkill, but ensures that the audio interface and Novation LaunchPad X connected to it both get enough power to light up and work their magic.

****** Sources 5 & 6 -- Raspberry Pi 4
If you prod it hard enough, a Raspberry Pi 4 will run OBS with the HTML Source option (it's not officially supported, but I managed to make it work somehow via a process I didn't document and can't remember), which means I can use it to run motion graphics if I don't mind not being able to capture the iPad screen. I do this when I have Zoom guests because the limitations of the ATEM mini mean I can't run them on the secondary mac screen, which holds the Zoom display.

Right now, this is far more fiddly than I'd like, and in the short to medium term, I plan to switch over to a Pi 5, if it'll fit in the same rack mount as the 4, which has substantially more horsepower for the stuff I need it to do. It's attached to the same bit of 1U high bent steel that the mac mini is attached to.

****** Sources 7 & 8 -- Gaming PC
It was definitely a gaming PC when I built it. It's coming up on eight years old though, so when I have the time, money and inclination I plan to rebuild it with a more modern processor and graphics card (which means a new motherboard -- essentially we're talking an entirely new PC). I've barely turned it on in the last year though, so there's really no rush.


***** Lighting
My key light is a daylight balanced GODOX 100W LED bouncing off a 60″ brolly and diffuser. Not as sexy as a big old parabolic softbox, but cheap, effective and it doesn't chew up as much space. The light is hanging from the arm of a C-stand and I really should attach a safety cable one of these days. My fill light is a PixaPro dual colour 60W circular LED panel in tungsten mode, so even though I end up with very low contrast lighting, there's a small colour temperature difference which fools the eye into thinking there's more actual contrast than there is. The panel is held by a Manfrotto magic arm clamped to the upright of a monitor stand.

When it's charged, I use a small Aputure MC10 set behind me as a hair/rim light.

***** Backdrop
I have a couple of lightstands holding a crossbar over which I've hung a duvet and kelim rug, held in place by a bunch of cheap A clamps. It's a bit precarious, but it mostly works, and I can't be arsed to set up a more professional backdrop hanging system. The duvet helps with audio as well -- it's a classic Sound on Sound tip for recording vocals in an untreated room and it works remarkably well -- there's no other sound treatment in the room.

**** Audio Gear

***** Interfaces
Oh, the interfaces I have known and loved! Right now, I have a couple of choices:

For live gigs, I have an SSL2+ that's USB bus powered, sounds great and slips in a small shoulder bag with the folded hub/stand, LaunchPad X, an XLR cable and my mic pouch stuffed with my mic and a bunch of short USB cables for attaching everything to the stand.

For streaming, I have an iConnectivity Audio 4c in my rack case. The 4c doesn't necessarily have the greatest preamps or analog to digital converters, but what it does have is two USB connections, so I can use it as an audio interface for my Mac and my iPad /at the same time/ and use that to send audio between the two, as well as sharing up to four mics (or instruments). I used to have an interface for the iPad and one for mac and I'd send audio between them with balanced jack cables, but that was annoyingly noisy (even with balanced cables) and just too many boxes. I snapped up a 4c as soon as I heard about them and it's been a game changer. Now my audio gets into the ATEM via Mac's two HDMI outputs and it stays digital and interference free all the way from Loopy to the viewer's computer.

I have a Focusrite 2Pre as well, and its preamps sound nicer than the 4c's and I could get clever with aggregate audio devices on the mac and use its pres instead of the 4c pres, but it's fiddly to rig and the 4c doesn't sound /awful./
...
:PROPERTIES:
:export_file_name: 20240301-emulating-ed-sheeran-s-looper-x-in-loopy-pro
:END:

I'm not sure if this will ever see the light of day, but what the hell, I'll do it here in case I can turn it into a proper write up.

#+hugo: more

At the moment, I'm only concerned with emulating Ed's 'multi' workflow. If I get that working, I'll hopefully have a leg up on implementing the other modes. Also, I'm assuming we're working with an unset tempo to begin with.
...
Press: Start recording into the selected Loop, go to Initial Recording State.

***** Initial Recording State

Press: Start overdubbing, go to Overdubbing state

***** Overdubbing state

We're in overdubbing state any time we want to record stuff after we've recorded the first layer of the first recorded clip.

Press: Finish overdubbing the active clip, go to Playing state

***** Playing state

Press: Start overdubbing selected clip. Go to Overdubbing state

***** Stopped state

Press: Start all clips. Go to Playing state
...
Press: Select associated Loop

****** Initial Recording State

Press: Select and start recording target loop. Goto Overdubbing state


****** Overdubbing State

Press: Select and start recording target loop. Remain in Overdubbing state

****** Playing State

Press: Select target loop. Remain in Playing state
...
In all states, just toggle muting of the target loop.


**** Stop

In both Arming and Muting mode, a long press will erase all loops, select the first, set the mode to Arming and the state to 'Initial'
...
Press: Stop the transport. Goto Stopped state.

*** State tables galore

So, the 8 buttons on the looper interact in some fiddly ways, primarily based on the state of the Record/Play button and the Mode button. I'm not worrying about the function button at the moment. Button actions available are Press and Hold. I /think/ Press triggers immediately even when the button is held.

In the initial looper state, the Rec/Play button's in the 'Playing' state, the Mode is 'Rec', Track 1 is selected, and all tracks are unmuted. This isn't a complete table at present, but I think it'll work as a start.

| Rec/Play | Mode   | Button:Action | Next R/P | Next Mode | Side effects                                                               |
|----------+--------+---------------+----------+-----------+----------------------------------------------------------------------------|
| Play     | Arming | TrackN:Tap    | Play     | Arming    | Arm Track N                                                                |
| Play     | Arming | TrackN:Hold   | Play     | Arming    | Peel Track N                                                               |
| Play     | Arming | Stop:Tap      | Play     | Arming    | Stop the clock, resetting the session                                      |
| *        | *      | Stop:Hold     | Play     | Arming    | Clear and reinitialize the session                                         |
| Play     | Arming | Mode:Tap      | Play     | Muting    |                                                                            |
| Play     | Muting | TrackN:Tap    | Play     | Muting    | Toggle Muting of Track N                                                   |
| Play     | Muting | TrackN:Hold   | Play     | Muting    | Don't Know                                                                 |
| Play     | *      | Rec/Play:Tap  | Rec/OD   | *         | Start recording/overdubbing selected track. Set Rec/Play state accordingly |
| Rec      | *      | Rec/Play:Tap  | OD       | *         | Finish initial recording of selected track, start overdubbing              |
| OD       | *      | Rec/Play:Tap  | Play     | *         | Finish overdubbing selected track immediately                              |
| Rec/OD   | Arming | TrackN:Tap    | Rec/OD   | Arming    | Start recording/overdubbing selected track. Set Rec/Play state accordingly |
|          |        |               |          |           |                                                                            |
...
| State Transition     | Follow Action         |
|----------------------+-----------------------|
| Start Initial Record | Rec/Play State -> Rec |
| Start Overdubbing    | Rec/Play State -> OD  |


*** Breaking it down to widgets

**** Button widgets

In general, our button widgets will delegate to stepped dials and will have the following bindings

| Gesture | Action                                 |
|---------+----------------------------------------|
| Press   | Nudge associated controller dial by +1 |
| Hold    | Nudge associated controller dial by +2 |

There are a couple of exceptions (at the moment at least)

***** Stop Button

The stop button is kind of the emergency stop button. Regardless of the state of the rest of the looper, it should stop everything. This will no doubt change as we add more modes, but for now:

| Gesture | Action                     |
|---------+----------------------------|
| Press   | Stop the clock             |
| Hold    | Wipe and reset the session |


**** Button Controllers

In general, each button controller is configured in groups of three. The first position of any group is the resting position for that state, +1 is the action to perform on a simple press, +2 is for a hold. These should always set the dial to a state label.


***** Track button controllers

All actions return the controller dial back to the state they started in


| Mode      | Press Action                               | Hold Action                             |
|-----------+--------------------------------------------+-----------------------------------------|
| Muting    | Mute the associated track                  | Peel the associated track               |
| Recording | Select this track and nudge Rec/Play by +3 | Select this track and nudge Rec/Play +4 |



***** Mode button controller

| Mode      | Press Action                             | Hold Action |
|-----------+------------------------------------------+-------------|
| Muting    | Set all track controllers to 'Recording' | ????        |
|           | Move this dial to 'Recording'            |             |
|-----------+------------------------------------------+-------------|
| Recording | Set all track controllers to 'Muting'    | ????        |
|           | Move this dial to 'Muting'               |             |


***** Rec/Play button controller

When we're in the play mode, the clock is stopped, there's at least one populated clip, and we're in Muting mode, tapping on the button should just restart the clock.

Rec/Play responds to more than one possible action, as well as the main button, it also gets informed when a track button is pressed

| State | Press Action                                   | Hold Action                           | Track Armed Action                   |
|-------+------------------------------------------------+---------------------------------------+--------------------------------------|
| Play  | Start recording the selected track             | Peel selected Track                   | Return to play position              |
|       | Let the track's follow action update our state | Return to Play position               |                                      |
|-------+------------------------------------------------+---------------------------------------+--------------------------------------|
| Rec   | Start overdubbing the selected track           | Start playing selected track and peel | Start recording selected track       |
|       | Let the Follow action update our state         | Return to Play position               | Let the follow action set state      |
|-------+------------------------------------------------+---------------------------------------+--------------------------------------|
| OD    | Finish recording selected track                | Finish overdubbing and peel selected  | Start recording/overdubbing selected |
|       | Move to Play position                          | Return to Play position               | Let the follow action work           |
** CANCELLED A few tips to get you started vocal looping
   CLOSED: [2024-10-19 Sat 16:19]
:PROPERTIES:
:export_file_name: 20240309-a-few-tips-to-get-you-started-vocal-looping
:END:
...
This is easily the most important lesson for acquiring any skill. When you start, you're going to suck. The secret is, even when you're experienced, there are still going to be times when you suck. The trick is not to let that phase you; chase the magic moments when you /don't/ suck, or the moments when you use that to learn something new about how not to suck. I /love/ finding new things that I enjoy doing that I suck at, because the great thing about sucking is that you get lots and lots of little dopamine kicks as you learn new things that help you not suck so badly. Once you're good at a thing, those "I learned something!" hits are more widely spaced (but, oh boy, are they great when you get one).

*** Count

Here's a great loop to get you started. Count: One, two, three, four at a steady pace, as you come back around to 'one', hit the record button and, just as you're about to say the next 'one', end the recording. What you want to hear, as it's playing back is:

#+begin_verse
One, two, three, four, One, two, three, four…
#+end_verse

Chances are, though, you'll hear
...
And that's okay, you're new to this, and what you're working on here is the fundamental skill of looping -- getting your loop point bang on. Once you can do that, it's all just experimentation and doing what interests you, so wipe the loop and try again. And again. And again. And again.

Eventually you'll be dialled in with your looper and its controls[fn:14] and you're ready for the next exciting step to learning to loop:

Start the loop playing and count along with it and, as you get back to the one again, start overdubbing for another four count and go back to playing. What you're aiming for is really tight timing. Listen especially for the 't' of the 'two' -- in a perfect world you should only hear one of them. If you're not happy with what you've got, tap undo and try again. Eventually, your overdubs will start sounding tight too, so try overdubbing multiple repeats and see how many times you can overdub before things start to sound muddy. The more layers you can add this way the better.

I can usually manage four or five repeats before I start to get really unhappy with the timing, but that's mostly okay when I'm actually singing because I'm also singing harmonies and that and reverb can cover for some less than stellar timing. Tighter is always better though.

Fancy a harder challenge? Start another 1,2,3,4 loop and this time, when you're overdubbing, just overdub the 'and's in "one and two and three and four and…" I'll bet that the first few times you try this you're going to get a swing feel going on, which is /great/ when it'd deliberate, but we both know it isn't, is it? Keep undoing and retrying until you're making a tight (kind of boring and foursquare) count and then start working on putting swing in deliberately. This is /hard!/ I'm still working on getting this as locked in as I'd like. If you do get to the point where you're happy with overdubbing the ands, try overdubbing the 'ee's and 'a's to end up with "one-ee and-a two-ee and-a three-ee and-a four-ee and-a" and bask in my admiration if you get good at that.

*** Embrace your inner crappy beatboxer

Unless you're aiming to become a dedicated beatboxer (and if you are, why are you reading this?) then don't
