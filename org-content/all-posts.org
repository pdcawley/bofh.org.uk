# -*- mode: org; mode: visual-line-mode; mode: visual-fill-column-mode; mode: org-hugo-auto-export-mode -*-
#+hugo_section: post/
#+hugo_base_dir: ../
...
If you're curious{{% marginnote %}} And several of you seem to be, so I wrote [[file:/2019/03/04/recursive-sql-recipes/][another post]] with a bit more detail and some sample code.{{% /marginnote %}} about the details of the schema, you can find it in my [[https://github.com/pdcawley/bakehouse][github repo]] for the bakery.
...
It turns out that the way a ~UNION~ works is akin to doing ~SELECT DISTINCT~ on the combined table, so it selects only unique rows. When two orders end up requiring exactly the same amount of the 'same' dough, they get smashed together and we lose half the weight. This is not ideal.{{% marginnote %}}It's /especially/ not ideal when you don't spot there's a problem and end up making far fewer loaves than you expect. Or on one /really/ annoying occasion, making a dough that was far too dry because we lost some water along the way. You can correct this during the mix, but it was a nasty shock.{{% /marginnote %}} I fixed it by adding a 'path' to the query, keeping track of how we arrived at a particular formula. Something like:
...
** The perils of writing stuff up
   :PROPERTIES:
   :ID:       4F2A8A82-DA19-41F8-9EDE-B554D29DEE82
   :END:
...
If we have that table, then two days before our bread is due, if we have an order for 10 white loaves, we'll need to mix \(9.3 × .288 \approxeq 2.68\) kg of flour and $9.3 × 0.173 \approxeq 1.61$ kg of water. Which we can do with a simple non-recursive ~SELECT~. Something like:{{% marginnote %}}NB: I've not tested this because I don't have the precalculated table, but it seems like it should work. In fact, thinking about it, we could probably build the ~precalc~ table so that we can simply do ~precalc.factor * po.quantity~, since any change that affects recipe weight will also affect our precalculated table.{{% /marginnote %}}
...
[[https://twitch.tv/telear][Twitch stream]] and I would  love for you to be there.
...
other independent musicians reach their audience through the platform.{{% marginnote %}}Recent deals with [[https://soundcloud.com][SoundCloud]] have made it /much/ easier for experienced
...
* DRAFT Stay (The Fuck) At Home                         :@repertoire:project:
  :PROPERTIES:
  :export_file_name: stay-at-home
  :ID:       7066F907-3B4C-437C-86E3-492B48391267
  :END:

  {{< embed "https://youtu.be/1RiGM6KrH30" />}}

As the lockdown started, some friends{{<marginnote>}}Vikki Appleton Fielden, Heather Charlton, Helen Edwards{{< /marginnote >}} and I wrote what we hoped would be a funny song about the best way to protect yourself and your loved ones as the enormity of the pandemic started to become apparent. Stay the fuck at home. We thought it would be a larky topical song, good for a couple of weeks as it became apparent that the government was being... hesitant, let's say about introducing a firm lockdown. A few digs at the "take it on the chin" or "herd immunity" policy that seemed to see the unnecessary deaths of thousands of elderly vulnerable people as acceptable losses. It didn't exactly go viral, but a bunch of Facebook friends all joined in, leading to the video you see above.
...
Well, it can. It just takes a little work.[fn:1] The ~question~ to ask is how do all those user friendly sites do there thing? Twitter and Facebook, being the walled garden behemoths that they are do it by dictating two different microformats{{% marginnote %}}
Of bloody course!
